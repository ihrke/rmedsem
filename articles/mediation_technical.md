...
There are two methods `rmedsem` uses as the basis for its procedures.
The first method is the commonly known Baron and Kenny approach which is
...
In case the model has been estimated using `blavaan`, posterior
estimates are used for deciding whether or not a test was “significant”
(HDI excluding or including zero).
...
Note: For models estimated with `cSEM`, the package uses bootstrapping
instead of Monte Carlo.

Note: For models estimated with `blavaan`, the package uses posterior
estimates.
